...
# This script makes figures 2.28, 2.30 and A.14, which show the ne profiles for
...
# Set this to True to overplot finite-difference approximations:
overplot_fd = False
...
for case in cases:
    ...
    mask = scipy.zeros_like(p.y, dtype=bool)
    mask[0] = True
    p.remove_points(mask)
    
    # Create GP:
    p.create_gp(constrain_at_limiter=False, k='SE')
    p.gp.k.hyperprior.bounds = [(0, 50.0), (0, 15.0)]
...
with open("ne_rotation.pkl", "wb") as pf:
...
a.set_title(r"(a) $\Delta\mathrm{d}^2n_{\mathrm{e}}/\mathrm{d}(r/a)^2$")

a_a_L_grad = f.add_subplot(3, 1, 2, sharex=a)
...
a_a_L_grad.set_title(r"(b) $\Delta a/L_{\nabla n_{\mathrm{e}}}$")
a_a_L_grad.set_ylabel(r"$\mathbb{P}$")

a_a2_2 = f.add_subplot(3, 1, 3, sharex=a)
...
a_a2_2.set_xlabel("$c$")
a_a2_2.set_title(r"(c) $\Delta \mathrm{d}^2n_{\mathrm{e}}/\mathrm{d}(r/a)^2/n_{\mathrm{e}}$")
a_a2_2.set_ylabel(r"$\mathbb{P}$")

f.subplots_adjust(hspace=0.25)
setupplots.apply_formatter(f)
f.savefig("prof_ne2_difference.pdf", bbox_inches='tight')
f.savefig("prof_ne2_difference.pgf", bbox_inches='tight')
...
for i, case in enumerate(cases):
    ...
    gptools.plot_sampler(
        case.sampler,
        labels=[r'$\sigma_f$ [$\SI{e20}{m^{-3}}$]', r'$\ell$'],
        burn=200,
        suptitle=r'$n_{\text{e}}$, %s' % (l,),
        bottom_sep=0.13,
        chain_ytick_pad=0.8,
        suptitle_space=0.1,
        label_fontsize=11,
    ...
    plt.savefig("%s-ne-marginals.pgf" % (l,), bbox_inches='tight')
    plt.savefig("%s-ne-marginals.pdf" % (l,), bbox_inches='tight')
    ...
    gptools.univariate_envelope_plot(
        ...
        case.out_MCMC['std_a2_2'][core_mask], ax=a_a2_2, linewidth=lwd,
        linestyle='-' if i == 0 else '--', color=c, lb=a2_2_bounds[0], ub=a2_2_bounds[1]
    )
    
    if overplot_fd:
        grad_approx = scipy.diff(case.out_MCMC['mean_val']) / scipy.diff(roa_star)
        a_grad.plot(
            roa_star[:-1], grad_approx,
            linewidth=lwd, linestyle=':', color=c
        )
        a_a_L.plot(
            roa_star[:-1], -grad_approx / case.out_MCMC['mean_val'][:-1],
            linewidth=lwd, linestyle=':', color=c
        )
        grad_2_approx = scipy.diff(grad_approx) / scipy.diff(roa_star[:-1])
        a_2.plot(
            roa_star[:-2], grad_2_approx, linewidth=lwd, linestyle=':', color=c
        )
        a_a_L_grad.plot(roa_star[:-2], grad_2_approx / grad_approx[:-1], linewidth=lwd, linestyle=':', color=c)
        a_a2_2.plot(roa_star[:-2], grad_2_approx / case.out_MCMC['mean_val'][:-2], linewidth=lwd, linestyle=':', color=c)
...
a_val.legend(loc='lower left')

a_val.set_title(r'(a) $n_{\text{e}}$ [$\SI{e20}{m^{-3}}$]')
a_grad.set_title(r'(b) $\mathrm{d}n_{\text{e}}/\mathrm{d}(r/a)$ [$\SI{e20}{m^{-3}}$]')
a_a_L.set_title(r'(c) $a/L_{n_{\text{e}}}$')
a_2.set_title(r'(d) $\mathrm{d}^2n_{\text{e}}/\mathrm{d}(r/a)^2$ [$\SI{e20}{m^{-3}}$]')
a_a_L_grad.set_title(r'(e) $a/L_{\nabla n_{\text{e}}}$')
a_a2_2.set_title(r'(f) $\mathrm{d}^2n_{\text{e}}/\mathrm{d}(r/a)^2/n_{\text{e}}$')
...
f.suptitle(r"$n_{\text{e}}$ profile, \textsc{se} covariance kernel")
f.subplots_adjust(left=0.07, bottom=0.09, right=0.97, top=0.9, wspace=0.17, hspace=0.26)
setupplots.apply_formatter(f)
f.canvas.draw()
f.savefig("prof2ne.pdf")
f.savefig("prof2ne.pgf")
...
post_sum = setupplots.generate_post_sum(
    ...
    [200,] * len(cases),
    [[[r'$n_{\text{e}}$', c.label, r'$\sigma_f$ [$\SI{e20}{m^{-3}}$]'], ['', '', r'$\ell$']] for c in cases]
)
with open('neRotPostSum.tex', 'w') as tf:
    tf.write(post_sum)
